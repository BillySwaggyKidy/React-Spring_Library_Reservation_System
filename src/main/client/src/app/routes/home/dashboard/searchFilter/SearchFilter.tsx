...
import { bookType, bookValuesType, filterDataType } from "@/src/constants/interfaces";
...
const SearchFilter = ({booksList} : {booksList : bookType[]}) => {
    ...
    const handleFilterFieldData = (id : string, value: bookValuesType ) => {
    ...
    const filterBooks = (book: bookType) : boolean => {
        ...
        const bookMatchFilterConditions = filterFields.every((filter)=>filter.condition(book[filter.objKeyRef as keyof bookType],filter.value));
        return searchMatchName && bookMatchFilterConditions;
    } 
