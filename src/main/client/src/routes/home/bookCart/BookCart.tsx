...
export default function BookCart() {
    ...
    return (
        <div className="h-full w-full flex flex-col justify-center items-center overflow-y-auto p-2">
            <div className="h-full w-3/4 grid grid-rows-9 grid-cols-1 bg-neutral-400/80 rounded-xl px-2">
                ...
                <div className="row-span-2 flex flex-row justify-end items-start">
                    <button disabled={cartContent.length == 0} className={`bg-green-500 text-white text-center text-xl m-4 px-2 rounded-xl ${cartContent.length > 0 ? 'cursor-pointer hover:bg-green-700' : "disabled:bg-green-500/50"}`} onClick={createNewBooksReservation}>Confirm Reserve</button>
                </div> 
            </div>
        </div>
    );
